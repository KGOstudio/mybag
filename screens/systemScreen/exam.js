...
function ExamSystem (props) {
    ...
    const _postHomeWork = () => {
        (async () => {
            ...
            try{
                ...
                const posthomework = await axios.post('/addExam', {
                    title,
                    comment,
                    counter,
                    class1,
    ...
    const _getHomeWork = (query) => {
        (async () => {
            ...
            try{
                ...
                const gethomework = await axios.get('/getExam', {
    ...
    const _delExam = (query) => {
        (async () => {

            try{

                const delHoework = await axios.delete('/delExam', {
        ...
        >
            ...
            {!aniOpen ? (
                ...
                <View style={{flexDirection: 'row'}}>
                    ...
                    <Text style={{fontSize: 19, fontWeight: 'bold', color: color.darkBlue,marginTop: 15,marginLeft: screenWidht * 0.3}}>class: {item.class1}</Text>
                </View>
                
                <Text style={{fontSize: 19, fontWeight: 'bold', color: color.blue,marginTop: 15,marginLeft: 7}}>{item.comment}</Text>

                <TouchableOpacity onPress={() => {setKey(item.class1); console.log(key); _delExam(); }}>
                    <View style={{backgroundColor: color.blue, padding: 3, borderRadius: 7,width:screenWidht * 0.15,alignItems: 'center',marginLeft: screenWidht * 0.7,marginTop: 15}}>
    ...
    return (
        <ImageBackground source={require('../../images/backHomescreen.jpg')} style={{width: screenWidht, height:screenHeight, padding: 7}} onLayout={async() => {
            ...
            onPress={async() => {setuseId(await AsyncStorage.getItem('userId')); console.log(userId); _getHomeWork();}}>Exams</Text>
