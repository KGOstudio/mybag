...
function Trip (props) {
    ...
    const _getHomeWork = () => {
        (async () => {
            ...
            try{
                ...
                const gethomework = await axios.get('/getTrip')
    ...
    return (
        <ImageBackground source={require('../../images/backHomescreen.jpg')} style={{width: screenWidht, height:screenHeight, padding: 7}} onLayout={async() => {
            ...
            <Text style={{fontSize: 50, fontWeight: 'bold', marginTop: 30,marginLeft: 7, color: color.white}}   
            onPress={async() => {setclass1(await AsyncStorage.getItem('class')); console.log(class1); _getHomeWork();}}>Trips</Text>
